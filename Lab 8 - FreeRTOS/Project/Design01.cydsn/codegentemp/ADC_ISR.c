...
CY_ISR(ADC_ISR_Interrupt)
{
    ...
    BaseType_t higher_priority = pdFALSE; // Used for scheduling
    
    //Grab the mutex for the latest ADC digital value,    
    xSemaphoreTakeFromISR(adc_mutex, &higher_priority);
    
    adcDV = ADC_GetResult16(0); //update the ADC value atomically
    
    xSemaphoreGiveFromISR(adc_mutex, &higher_priority); //Release mutex for LCD update
    /* `#END` */
}
...
void ADC_ISR_SetPriority(uint8 priority)
{
    ...
    *ADC_ISR_INTC_PRIOR = (*ADC_ISR_INTC_PRIOR & (uint32)(~(uint32)ADC_ISR__INTC_PRIOR_MASK)) |
