...
#include "DAC_OUT.h"
    ...
    #define DAC_OUT_SetP4PinDriveMode(shift, mode)  \
    do { \
        DAC_OUT_PC =   (DAC_OUT_PC & \
                                (uint32)(~(uint32)(DAC_OUT_DRIVE_MODE_IND_MASK << \
                                (DAC_OUT_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (DAC_OUT_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define DAC_OUT_SetP4PinDriveMode(shift, mode)  \
        do { \
            DAC_OUT_USBIO_CTRL_REG = (DAC_OUT_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(DAC_OUT_DRIVE_MODE_IND_MASK << \
                                    (DAC_OUT_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (DAC_OUT_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		DAC_OUT_SetP4PinDriveMode(DAC_OUT__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(DAC_OUT_DR & (uint8)(~DAC_OUT_MASK));
    drVal = (drVal | ((uint8)(value << DAC_OUT_SHIFT) & DAC_OUT_MASK));
    DAC_OUT_DR = (uint32)drVal;
}
...
{
    return (uint8)((DAC_OUT_PS & DAC_OUT_MASK) >> DAC_OUT_SHIFT);
}
...
{
    return (uint8)((DAC_OUT_DR & DAC_OUT_MASK) >> DAC_OUT_SHIFT);
}
...
void DAC_OUT_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  DAC_OUT_INTCFG & (uint32)(~(uint32)position);
    DAC_OUT_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(DAC_OUT_INTSTAT & DAC_OUT_MASK);
	DAC_OUT_INTSTAT = maskedStatus;
    return maskedStatus >> DAC_OUT_SHIFT;
}
